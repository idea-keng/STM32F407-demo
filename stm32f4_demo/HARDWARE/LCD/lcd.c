	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				...
				else LCD_ShowChar(x+(size/2)*t,y,'0',size,mode&0X01);  
 				continue;
			}else enshow=1; 
...
//num:汉字字模编号	
//size:汉字字号	
void LCD_Show_ch(u16 x,u16 y,u16 width,u16 height,u8 size,u8 num)
{  
	u8 temp,t1,t,line,line_t;
	u16 y0=y;
	u16 yo=y;
	u16 xo=x;
	
	line=size/8;
	for(line_t=0;line_t<line;line_t++)
	{
		x=xo;
		y0=y=yo+8*line_t;
		for(t=0;t<size;t++)
		{   
			switch(size)
			{
				case 16:temp=ch_font_16[num][t+size*line_t];break;
				case 32:temp=ch_font_32[num][t+size*line_t];break;
				case 64:temp=ch_font_64[num][t+size*line_t];break;
...
//size:字号	
//len:字符串长度	
void LCD_Show_chstr(u16 x,u16 y,u8 size,int num[],u8 len)
{  
	u8 i;
	
	for(i=0;i<len;i++)
	{
		LCD_Show_ch(x+i*size,y,size,size,size,num[i]);
	}
}
...
//x,y:起点坐标 
//width,height:区域大小  
void LCD_Show_bmp(u16 x,u16 y,u16 width,u16 height,u8 *bmp)
{
	u8 temp,t1,t,line,line_t;
	u16 y0=y;
	u16 yo=y;
...
void LCD_Showfloat(u16 x,u16 y,float num,u8 len_z,u8 len_x,u8 size,u8 mode)
{  
	u16 temp=num;

	if(num>=0){
		LCD_ShowChar(x,y,' ',size,0);
		LCD_ShowNum(x+12,y,num,len_z,size);
		LCD_ShowChar(x+12+(size/2)*len_z,y,'.',size,0);
		num-=temp;
		num*=LCD_Pow(10,len_x);
		LCD_ShowxNum(x+12+(size/2)*(len_z+1),y,num,len_x,size,mode);
	}
	else{
		temp=num=-num;
		LCD_ShowChar(x,y,'-',size,0);
